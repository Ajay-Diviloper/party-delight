...
const ProductEnquiryModal: React.FC<ProductEnquiryModalProps> = ({ productName }) => {
  ...
  return (
    <>
      <button
        className="bg-black text-white font-heading px-8 py-3 rounded-full text-base tracking-wide hover:bg-[#ff3131] transition"
      ...
      {showModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md relative">
            ...
            <h2 className="font-heading text-2xl mb-4 text-[#ff3131]">Product Enquiry</h2>
            <form
              className="space-y-5"
              onSubmit={e => {
                e.preventDefault();
